...
Middleware components in ASP.NET Core are used to process HTTP requests and perform various tasks such as authentication, logging, error handling, and routing. The order in which middleware components are registered is important as it determines the processing flow of requests.

## Usage of UseMiddleware Extension Method

The `UseMiddleware` extension method is used to add middleware components to the request pipeline. It is typically called within the `Configure` method of the `Startup` class.
...
Middleware is a key component in web development that sits between the web server and the application. It handles incoming requests and outgoing responses, allowing developers to add additional functionalities and modify the request/response pipeline. The middleware pipeline consists of a chain of components, each responsible for a specific task.

## InvokeAsync or Invoke Methods

The `InvokeAsync` or `Invoke` methods are the main entry points for the middleware pipeline. They are responsible for processing the incoming request and generating the outgoing response. These methods are usually implemented by middleware components and are called in a sequential manner.

### Parameters

1. `HttpContext`: The `HttpContext` parameter represents the context of the current HTTP request. It provides access to the request and response objects, as well as other important information such as the user, session, and server variables.

2. `RequestDelegate`: The `RequestDelegate` parameter is a callback that represents the next middleware component in the pipeline. It is responsible for invoking the next middleware and passing the request to it.

## Pre and Post-Processing of Request/Response

Middleware components can perform pre-processing and post-processing tasks on the incoming request and outgoing response, respectively. This allows developers to modify the request/response pipeline based on their specific requirements.

Pre-processing tasks commonly performed by middleware include authentication, authorization, request parsing, request validation, and logging. These tasks are executed before passing the request to the next middleware component.

Post-processing tasks include response modification, content transformation, error handling, and logging. These tasks are executed after the response is generated by the application but before it is sent back to the client.

Middleware components can intercept the request and response at various stages of the pipeline, enabling developers to add custom logic and modify the behavior of the application.
